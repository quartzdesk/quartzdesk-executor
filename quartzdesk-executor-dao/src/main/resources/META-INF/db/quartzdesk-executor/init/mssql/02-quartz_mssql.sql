...
-- GO statements replaced with semicolons (required by the DatabaseScriptExecutor).
--

IF EXISTS( SELECT
             *
           FROM DBO.SYSOBJECTS
           WHERE ID = OBJECT_ID( N'[DBO].[FK_QRTZ_TRIGGERS_QRTZ_JOB_DETAILS]' ) AND
...
IF EXISTS( SELECT
             *
           FROM DBO.SYSOBJECTS
           WHERE ID = OBJECT_ID( N'[DBO].[FK_QRTZ_CRON_TRIGGERS_QRTZ_TRIGGERS]' ) AND
                 OBJECTPROPERTY( ID, N'ISFOREIGNKEY' ) = 1 )
